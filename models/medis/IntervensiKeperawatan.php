 ...
 */
class IntervensiKeperawatan extends \yii\db\ActiveRecord
{
    /**
     * {@inheritdoc}
     */
    ...
    /**
     * {@inheritdoc}
     * @return IntervensiKeperawatanQuery the active query used by this AR class.
     */
    public static function find()
    {
        return new IntervensiKeperawatanQuery(get_called_class());
    }
    public static function getListIntervensi($q=null){
        ...
        select
            id as id ,deskripsi as text, rumpun,CONCAT('(',rumpun,')',deskripsi) as text_rumpun,deskripsi
        FROM parents icd  where icd.id not in (select distinct parent from parents where parent is not null) AND (LOWER(deskripsi) LIKE '%".strtolower($q)."%')
        ";

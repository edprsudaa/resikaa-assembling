...
class TbPegawai extends \yii\db\ActiveRecord
{
    ...
    /**
     * {@inheritdoc}
     * @return TbPegawaiQuery the active query used by this AR class.
     */
    ...
    // {
    //     return $this->hasMany(TbRiwayatPenempatan::className(),['id_nip_nrp'=>'id_nip_nrp'])->orderBy([TbRiwayatPenempatan::tableName().'.tanggal'=>SORT_DESC]);
    // }
    // public function getUnitPltPlh()
    // {
    //     return $this->hasMany(TbUnitPltPlh::className(),['id_nip_nrp'=>'id_nip_nrp'])->orderBy([TbUnitPltPlh::tableName().'.tanggal_surat'=>SORT_DESC]);
    // }
    public function getRiwayatStr()
    {
        return $this->hasMany(TbRiwayatStr::className(),['id_nip_nrp'=>'id_nip_nrp'])->orderBy([TbRiwayatStr::tableName().'.tanggal_terbit'=>SORT_DESC]);
    }
    public function getRiwayatSipp()
    {
        return $this->hasMany(TbRiwayatSipp::className(),['id_nip_nrp'=>'id_nip_nrp'])->orderBy([TbRiwayatSipp::tableName().'.tanggal_terbit'=>SORT_DESC]);
    }
    public function getRiwayatSpklinis()
    {
        return $this->hasMany(TbRiwayatSpklinis::className(),['id_nip_nrp'=>'id_nip_nrp'])->orderBy([TbRiwayatSpklinis::tableName().'.tanggal_terbit'=>SORT_DESC]);
    }
    public function getRiwayatPenempatan()
    {
        return $this->hasMany(TbRiwayatPenempatan::className(),['id_nip_nrp'=>'id_nip_nrp']);
    }
    public function getPltPlh()
    {
        return $this->hasMany(TbUnitPltPlh::className(),['id_nip_nrp'=>'id_nip_nrp']);
    }
    public static function getNamaDokter($id)
    {
        return self::find()->select([
            "coalesce(concat(CASE WHEN gelar_sarjana_depan='dr' THEN 'dr.' ELSE gelar_sarjana_depan END, ' ') , '') || coalesce(INITCAP(nama_lengkap), '') || coalesce(concat(' ', gelar_sarjana_belakang), '') as dokter_nama"
        ])
            ->where(['pegawai_id' => $id])->asArray()->one();
    }

 ...
 */
class LaporanKetidakTepatanWaktu extends \yii\db\ActiveRecord
{
    ...
    function saveData()
    {
        $analisaDokumenReq = Yii::$app->request->post('AnalisaDokumen');
        $analisaDokumenReqDetail = Yii::$app->request->post('AnalisaDokumenDetail');
        ...
        try {
            if (Yii::$app->request->post('analisa_dokumen_id')) {
                $analisaDokumen = AnalisaDokumen::find()->where(['analisa_dokumen_id' => Yii::$app->request->post('analisa_dokumen_id')])->one();
                ...
                if ($analisaDokumen->save()) {
                    foreach ($analisaDokumenReqDetail as $k) {
                        $data = AnalisaDokumenDetail::find()->where([
                ...
                foreach ($analisaDokumenReqDetail as $k) {
                    ...
                    // Yii::$app->db->createCommand()->insert(AnalisaDokumenDetail::tableName(), [
                    //     "analisa_dokumen_id" => $this->analisa_dokumen_id,
                    //     "analisa_dokumen_jenis_analisa_detail_id" => $k['analisa_dokumen_jenis_analisa_detail_id'],
                    //     "analisa_dokumen_item_id" => $k['analisa_dokumen_item_id'],
                    //     "analisa_dokumen_jenis_id" => $k['analisa_dokumen_jenis_id'],
                    //     "analisa_dokumen_kelengkapan" => $k['analisa_dokumen_kelengkapan'],
                    //     "analisa_dokumen_dokter_id" => Yii::$app->request->post('analisa_dokumen_dokter_id'),
                    //     "analisa_dokumen_petugas_analisa_id" => Yii::$app->user->identity->id,
    ...
    {
        return $this->hasMany(AnalisaDokumenDetail::className(), ['analisa_dokumen_id' => 'analisa_dokumen_id']);
    }
}

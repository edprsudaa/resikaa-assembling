...
// Event handler untuk perubahan dropdown Select2
...
function myTable() {
  ...
  $.ajax({
    url: baseUrl+'/analisa-kuantitatif-ri/daftar-analisa-kuantitatif',
    type: 'post',
    ...
    success: function (response) {
      ...
      $.each(response.data, function (i, item) {
        ...
        let poliList = '';
        $.each(item.poliList, function(index, value) {
        ...
        trHTML +='<tr class=\'distribusi\' ><td>'+(x)+'</td><td>'+item.kode+'<br>'+item.pasien_kode+'<br>'+item.nama+'</td><td>'+item.tgl_masuk+'</td><td>'+item.tgl_keluar+'</td><td>'+poliList+'</td><td>'+analisa+'</td><td><a class=\'btn btn-sm btn-success\' href='+baseUrl+'/analisa-kuantitatif-ri/view?id='+item.registrasi_kode_hash+' target=\'_blank\' data-method=\'post\'><span class=\'nav-icon fas fa-edit text-white\' title=\'\' data-toggle=\'tooltip\' data-placement=\'bottom\' data-original-title=\'Coding Pelaporan\' aria-describedby=\'tooltip537870\'></span>Klik Untuk Analisa</a></td></tr>';

...
$('.btn-preview-resume-rj').click(function(e){
    ...
    console.log(pasien)
	if(id){
		$.post('" . Url::to(['analisa-kuantitatif/detail-resume-rj']) . "',{id:id,pasien:pasien},function(res){
...
$('.btn-sbpk').click(function(e){
	...
	var id=$(this).attr('data-id');
	if(id){
		$.post('" . Url::to(['analisa-kuantitatif/detail-sbpk']) . "',{id:id},function(res){
			$('.mymodal_card_xl_body').html(res);
            $('.mymodal_card_xl').modal('show');
		});
	}
...
<div class="row">
    <div class="col-lg-12">
        <!-- Card Pasien -->
        <?= $this->render('card-pasien', ['registrasi' => $registrasi]); ?>
        <div class="row">
            ...
            <div class="col-lg-12">
                ...
                echo $this->render('form_skrining_pasien_mpp', ['model' => $model, 'listSkriningPasienMpp' => $listSkriningPasienMpp, 'listMpp' => $listMpp, 'registrasi' => $registrasi]);
                ?>
            </div>

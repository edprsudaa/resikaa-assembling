  ...
  $.ajax({
    ...
    success: function (response) {
      ...
      $.each(response.data, function (i, item) {
        ...
        let aksi_closing='';
        if(item.closing != 0){
          aksi_closing += '<span class=\'badge badge-warning\'>Closing Tgl '+item.tgl_closing+'<br>By Adm '+item.adm+'</span><br>';
        }else{
          aksi_closing += '<a class=\'btn btn-sm btn-success\' href=\'/mpp/catatan?id='+item.registrasi_kode_hash+'&amp;layanan_id='+item.layanan_id+'&amp;layanan_nama='+item.layanan_nama+'\'><span class=\'nav-icon fas fa-edit text-white\' title=\'\' data-toggle=\'tooltip\' data-placement=\'bottom\' data-original-title=\'Coding Pelaporan\' aria-describedby=\'tooltip537870\'></span> Pilih Pasien</a>';

        }
        trHTML +='<tr class=\'distribusi\' ><td>'+(i+1)+'</td><td>'+item.nomor_registrasi+'<br>'+item.rm_pasien+'<br>'+item.nama_pasien+'<br>'+item.rawat+'</td><td>'+item.tgl_registrasi+'</td><td>'+item.tgl_checkout+'</td><td>'+item.tgl_masuk+'</td><td>'+item.tgl_keluar+'</td><td>'+item.unit+'</td><td>'+item.unit_asal+'</td><td>'+item.catatan+'</td><td>'+item.resume+'</td><td>'+aksi_closing+'</td></tr>';
  ...
  <div class="col-lg-12">
    <div class="card card-primary card-outline">
      <div class="card-body">
        ...
        <table id="example1" style="width: 100%;" class="table table-striped table-responsive">
          <thead>
            <tr style="background-color: #0BB783;color: white;">
              ...
              <th width="10%">Poli/Unit</th>
              <th width="5%">Poli/Unit Asal</th>

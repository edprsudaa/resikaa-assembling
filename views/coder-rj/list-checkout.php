                    ...
                    'columns' => [
                            ...
                            'value' => function ($model) {
                                ...
                                foreach ($model->layanan as $no => $item) {
                                    if ($item->jenis_layanan == 1) {
                                        $data .= '<span class="badge bg-danger">' . ($no + 1) . '. Instalasi Gawat Darurat - ' . $item->tgl_masuk . '</span><br>' ?? '';
                                    } elseif ($item->jenis_layanan == 2) {
                                        $data .= '<span class="badge bg-success">' . ($no + 1) . '. Rawat Jalan (' . $item->unit->nama . ') - ' . $item->tgl_masuk . '</span><br>' ?? '';
                                    } elseif ($item->jenis_layanan == 3) {
                                        $data .= '<span class="badge bg-info">' . ($no + 1) . '. Rawat Inap (' . $item->unit->nama . ') - ' . $item->tgl_masuk . '</span><br>' ?? '';
                                    }
                                }
                                return $data;
                            },
                        ...
                        [
                            ...
                            'filter' => Select2::widget([
                                'model' => $searchModel,
                                'attribute' => 'is_analisa',
                                'data' => [1 => 'Sudah Analisa', 0 => 'Belum Analisa'],
                                'options' => [
                                    'placeholder' => 'Pilih...'
                                ],
                                'pluginOptions' => [

...
$this->title = 'Daftar Kunjungan Pasien ' . ($pasien != NULL ? $pasien['nama'] . ' (' . $pasien['kode'] . ')' : '');
// echo '<pre>';
// print_r($pasien);
// echo '</pre>';
// die;
                    ...
                    <?php foreach ($registrasi as $key => $item) { ?>
                        <div class=" callout callout-danger">
                            <a class="nav-link bg-info  <?php if ($key == 0) {
                                                            echo 'active';
                                                        } ?>" id="vert-tabs-profile-tab<?= $key ?>" data-toggle="pill" data-href=<?= Url::to(["history-pasien/detail-kunjungan?noreg=" . HelperGeneralClass::hashData($item["kode"])]); ?> href="#vert-tabs-profile<?= $key ?>" role="tab" aria-controls="vert-tabs-profile<?= $key ?>" aria-selected="false">

...
class AnalisaKuantitatifIgdController extends Controller
{
    /**
     * {@inheritdoc}
     */
    ...
    public function actionDaftarAnalisaKuantitatif()
    {
        ...
        if ($req['tanggal_awal'] != null) {
            $queryData = $queryData->andWhere([">=", "registrasi.tgl_masuk", $tanggal_awal . " 00:00:00"]);
        } else {
            $queryData = $queryData->andWhere([">=", "registrasi.tgl_masuk", date('Y-m-d') . " 00:00:00"]);
        }
        if ($req['tanggal_akhir'] != null) {
            $queryData = $queryData->andWhere(["<=", "registrasi.tgl_masuk", $tanggal_akhir . " 23:59:59"]);
        } else {
        ...
        $queryDataTest = (new \yii\db\Query())
            ->select([
                ...
                "registrasi.is_analisa as analisa"
        ...
        if ($req['analisa'] != null) {
            if ($req['analisa'] == 1) {
                $queryDataTest = $queryDataTest->andWhere(["registrasi.is_analisa" => 1]);
            } elseif ($req['analisa'] == 0) {
                $queryDataTest = $queryDataTest->andWhere(["registrasi.is_analisa" => 0]);
            }
        }
    ...
    public function actionView($id = null, $icd = false)
    {
        ...
        $analisaDokumen = AnalisaDokumenIgd::find()->where(['reg_kode' => $registrasi->data['kode']])->limit(1)->one();

        if (!empty($analisaDokumen)) {
            $analisaDokumen = AnalisaDokumenIgd::find()
                ->innerJoin('analisa_dokumen_igd_detail', 'analisa_dokumen_igd_detail.analisa_dokumen_id=analisa_dokumen_igd.analisa_dokumen_id')
                ->where(['reg_kode' => $registrasi->data['kode']])->limit(1)->orderBy([
                    AnalisaDokumenIgdDetail::tableName() . '.analisa_dokumen_item_id' => SORT_ASC,
                    AnalisaDokumenIgdDetail::tableName() . '.analisa_dokumen_jenis_id' => SORT_ASC
                ])->one();
        } else {
            $analisaDokumen = new AnalisaDokumenIgd();
        }

        $listAnalisa = MasterJenisAnalisaDetail::find()->innerJoinWith(['jenisAnalisa', 'itemAnalisa'])->where(['jenis_analisa_detail_aktif' => 1])->andWhere(['jenis_analisa_detail_igd' => 1])->orderBy(['jenis_analisa_detail_jenis_analisa_id' => SORT_ASC, 'jenis_analisa_detail_item_analisa_id' => SORT_ASC])->asArray()->all();
    ...
    public function actionSave()
    {
        ...
        if ($req->isAjax) {
            ...
            if ($id != NULL) {
                $model = AnalisaDokumenIgd::find()->where(['ps_kode' => $req->post('AnalisaDokumenIgd')['ps_kode'], 'reg_kode' => $req->post('AnalisaDokumenIgd')['reg_kode']])->limit(1)->one();
            } else {
                $model = new AnalisaDokumenIgd();
            }
            $model->load($req->post());
    ...
    public function actionCetakAnalisa()
    {
        ...
        if ($model->load(Yii::$app->request->post())) {
            ...
            $query = AnalisaDokumenIgdDetail::find()
                ...
                ->innerJoin('master_jenis_analisa', 'analisa_dokumen_igd_detail.analisa_dokumen_jenis_id=master_jenis_analisa.jenis_analisa_id')
                ->innerJoin('master_jenis_analisa_detail', 'master_jenis_analisa_detail.jenis_analisa_detail_id=analisa_dokumen_igd_detail.analisa_dokumen_jenis_analisa_detail_id');
            // ->where(['analisa_dokumen_igd.dokter_id'=>'38'])
